...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	archivedCr := ptr.Deref(cr.Spec.ForProvider.Archived, false)
	...
	if !*repo.Fork {
		privateCr := ptr.Deref(cr.Spec.ForProvider.Private, true)
		if privateCr != *repo.Private {
			return notUpToDate, nil
		}
	}

	isTemplate := ptr.Deref(cr.Spec.ForProvider.IsTemplate, false)
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	if repo.Fork != nil && !*repo.Fork {
		val := ptr.Deref(cr.Spec.ForProvider.Private, true)
		privateCr = &val
	}
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) error {
	...
	forceDelete := ptr.Deref(cr.Spec.ForProvider.ForceDelete, false)
	if !forceDelete {
